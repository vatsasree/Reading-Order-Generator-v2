...
class SamPredictor:
    ...
    def predict_torch(
        ...
        if not hier_det:
            ...
            fg_points_num = 1500
            fg_mask = (high_res_masks > self.model.mask_threshold).squeeze(1)[0]
            ...
            point_coords = torch.cat((x_idx, y_idx), dim=1)[:, None, :]  # (k, 1, 2)
            image_size = (1024,1024)
            image = torch.zeros(image_size)
            pts=2
            point_coords = point_coords[:pts]
            point_labels = torch.ones(pts).unsqueeze(1)
            import cv2
            # Mark the points in the image
            for point in point_coords:
                x, y = point[0, 0].item(), point[0, 1].item()  # Extract x and y values
                if 0 <= x < image_size[1] and 0 <= y < image_size[0]:
                    image[y, x] = 1  # Set the pixel at (x, y) to white (value 1)
            
            image_np = image.numpy()
            image_np = (image_np * 255).astype(np.uint8)
            cv2.imwrite('generated_image.png', image_np)
